...
// AHP 전용 PortOne(V1) 결제검증 — orders 테이블, {paymentId(=imp_uid), orderId(=order_number)} 입력
...
serve(async (req) => {
  ...
  try {
    // 프론트(orderService.verifyPayment): { paymentId(=imp_uid), orderId(=orders.order_number) }
    ...
    // 1) 주문 조회 — 금액의 신뢰 원천은 클라이언트가 아니라 DB. orderId == order_number.
    ...
    if (tokenData.code !== 0) throw new Error('PortOne token failed: ' + JSON.stringify(tokenData));
    const accessToken = tokenData.response.access_token;

    // 3) 실제 결제 정보 조회 (paymentId = imp_uid)
    const payRes = await fetch(`https://api.iamport.kr/payments/${paymentId}`, {
      headers: { Authorization: accessToken },
    });
    const payData = await payRes.json();
    if (payData.code !== 0) throw new Error('PortOne payment lookup failed: ' + JSON.stringify(payData));
    ...
    if (!statusOk || !amountOk) {
      ...
      await supabase.from('orders')
        ...
        .eq('order_number', orderId);
      return new Response(
        JSON.stringify({ verified: false, error: statusOk ? '결제 금액 불일치' : '미결제 상태', paymentStatus: payment.status, paymentAmount: payment.amount, orderAmount: order.total_amount }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    // 5) 검증 통과 → service_role로 paid 설정
    const { error: updErr } = await supabase.from('orders').update({
      payment_status: 'paid',
      portone_payment_id: paymentId,
      paid_at: new Date().toISOString(),
    }).eq('order_number', orderId);
    if (updErr) throw new Error('Order update failed: ' + JSON.stringify(updErr));
