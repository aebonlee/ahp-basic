# 2026-02-24 — 설문 설계 구글 폼 스타일 개편 + 4단계 탭 UI + 플랫폼 가이드

> **날짜**: 2026-02-24
> **커밋**: 4건 (`206f848`, `a44b4a1`, `0a3628c`, `TBD`)
> **변경 파일**: 8개 (코드) + 1개 (개발일지)
...
| `TBD` | 오른쪽 사이드바 3탭 가이드 개편 + 플랫폼 가이드 추가 |
...
## 세션 4 — 오른쪽 사이드바 3탭 가이드 개편

### 핵심 변경: 2탭(연구자/평가자) → 3탭(플랫폼/연구자/평가자) + 타이틀 추가

```
┌──────────────────────────────────────┐
│   AHP 연구 플랫폼 가이드              │  ← 새 타이틀
├──────────────────────────────────────┤
│ [플랫폼] │ [연구자] │ [평가자]        │  ← 3탭 (기존 2탭에서 확장)
├──────────────────────────────────────┤
│                                      │
│  ① ② ③ ④ ⑤  (섹션 탭)              │
│                                      │
...
| 섹션 | 내용 |
|:----:|------|
| **1. 플랫폼 소개** | AHP 개요, 주요 기능 (프로젝트/설문/평가/집계/자원배분) |
| **2. 설문 설계** | 4단계 빌더 사용법, 7가지 질문 유형 설명 |
| **3. 평가 관리** | 평가자 초대, 평가 프로세스 5단계 |
| **4. 결과 활용** | 자동 집계, 민감도 분석, 자원 배분 |
| **5. 데이터 보안** | RLS 보안, 연구 윤리/IRB 준수 |

### ProjectLayout 변경

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| 타이틀 | 없음 | "AHP 연구 플랫폼 가이드" |
| 탭 수 | 2개 (연구자/평가자) | 3개 (플랫폼/연구자/평가자) |
| 기본 탭 | 연구자 | 플랫폼 |
| 접힌 상태 | R / E | P / R / E |
| 탭 구현 | 하드코딩 | GUIDE_TABS 배열 + map |

### 기존 유지

- ResearcherGuide (5섹션) — 변경 없음
- EvaluatorGuideSidebar (5섹션) — 변경 없음
...
| `src/components/admin/PlatformGuide.jsx` | 플랫폼 가이드 컴포넌트 (5섹션) |
| `src/components/admin/PlatformGuide.module.css` | 플랫폼 가이드 CSS |
| `src/components/layout/ProjectLayout.jsx` | 3탭 + 타이틀 개편 |
| `src/components/layout/ProjectLayout.module.css` | guideTitle 스타일 추가 |
